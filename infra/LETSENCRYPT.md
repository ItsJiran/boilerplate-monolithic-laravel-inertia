...
Citra Kuliner uses **Let's Encrypt** for SSL certificates in **PRODUCTION** environment.
...
1. **Public Domain**: You must own a domain (e.g., `citrakuliner.com`)
...
sudo certbot --nginx -d citrakuliner.com -d *.citrakuliner.com --manual --preferred-challenges dns
```
...
services:
  certbot:
    ...
    container_name: citrakuliner-certbot
    volumes:
...
docker-compose run --rm certbot certonly --webroot \
  -w /var/www/certbot \
  -d citrakuliner.com \
  -d www.citrakuliner.com \
  --email admin@citrakuliner.com \
  --agree-tos \
  --no-eff-email
...
services:
  traefik:
    image: traefik:v2.10
    container_name: citrakuliner-traefik
  ...
  app:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.app.rule=Host(`citrakuliner.com`)"
...
```nginx
# /etc/nginx/sites-available/citrakuliner.com

# HTTP - Redirect to HTTPS
server {
    listen 80;
    server_name citrakuliner.com www.citrakuliner.com;
...
server {
    ...
    # SSL Configuration
    ssl_certificate /etc/letsencrypt/live/citrakuliner.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/citrakuliner.com/privkey.pem;
...
SSL_CERT_PATH=/etc/letsencrypt/live/citrakuliner.com/fullchain.pem
SSL_KEY_PATH=/etc/letsencrypt/live/citrakuliner.com/privkey.pem
...
# Check expiration
openssl x509 -in /etc/letsencrypt/live/citrakuliner.com/cert.pem -noout -dates
```
...
```bash
# If compromised
sudo certbot revoke --cert-path /etc/letsencrypt/live/citrakuliner.com/cert.pem
```
...
dig citrakuliner.com
nslookup citrakuliner.com
...
cp infra/nginx/default.host.vps.template /etc/nginx/sites-available/citrakuliner.com

# Enable site
sudo ln -s /etc/nginx/sites-available/citrakuliner.com /etc/nginx/sites-enabled/
...
```bash
# Test HTTPS
curl -I https://citrakuliner.com

# Check SSL certificate
openssl s_client -connect citrakuliner.com:443 -servername citrakuliner.com
```
